...
// Adjustment of Proposal Variances
     if(imod(i,50)==0){
		...
		for (j=0; j<dim1; ++j){
		    if (accmu1[j]>22){Vmu1[j]=Vmu1[j]*exp(adj);} else {Vmu1[j]=Vmu1[j]*exp(-adj);}
			 if (acceta1[j]>22){Veta1[j]=Veta1[j]*exp(adj);} else {Veta1[j]=Veta1[j]*exp(-adj);}
			accmu1[j]=0; acceta1[j]=0;}
		for (j=0; j<dim2; ++j){
		    ...
		    accxi1=0; accsigma1=0; accu1=0; accxi2=0; accsigma2=0; accu2=0; accv=0;}			
